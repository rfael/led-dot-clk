...
use crate::{
    bsp::{Max7219, SharedDevice, SpiDev},
    impl_from_variant,
};
...
impl Display {
    ...
    pub async fn set_intensity(&self, intensity: u8) -> DisplayResult<()> {
        self.max7219
            .lock()
            .await
            .set_intensity(intensity & 0x0F)
            .await?;
        Ok(())
    }

    #[allow(unused)]
    pub async fn write_str(&self, text: &str) -> DisplayResult<()> {
        self.max7219.lock().await.write_str(text).await?;
        Ok(())
    }
    ...
    async fn write_hh_mm(&self, hour: u8, minute: u8) -> DisplayResult<()> {
        ...
        s.push((b'0' + minute % 10) as char)
            .map_err(|_| DisplayError::Format)?;
        self.max7219.lock().await.write_str(&s).await?;
        self.show_clock_dots(true).await?;
        Ok(())
    }

    async fn show_clock_dots(&self, show: bool) -> DisplayResult<()> {
        self.max7219.lock().await.set_pixel(15, 5, show).await?;
        self.max7219.lock().await.set_pixel(15, 3, show).await?;
        Ok(())
    }
